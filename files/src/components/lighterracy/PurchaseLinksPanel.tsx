...
const CHANNEL_META: Record<string, ChannelMeta> = {
  periplus: {
    ...
    shellClassName:
      "border-red-200 bg-gradient-to-br from-red-50 via-white to-rose-50 text-neutral-950",
...
function BrandMark({ meta }: { meta: ChannelMeta }) {
  if (meta.brand === "periplus") {
    return (
      <div className="inline-flex max-w-full flex-col rounded-lg bg-red-600 px-3 py-2 text-white shadow-sm">
        <span className="whitespace-nowrap text-[15px] font-black uppercase leading-none tracking-[0.04em] sm:text-[16px]">
          PERIPLUS.COM
        </span>
        <span className="mt-0.5 whitespace-nowrap text-[10px] font-semibold leading-none text-white/95 sm:text-[11px]">
  ...
  if (meta.brand === "tokopedia") {
    return (
      <div className="inline-flex max-w-full items-center gap-2 rounded-full bg-emerald-600 px-3 py-2 text-sm font-black text-white shadow-sm">
  ...
  if (meta.brand === "shopee") {
    return (
      <div className="inline-flex max-w-full items-center gap-2 rounded-full bg-orange-600 px-3 py-2 text-sm font-black text-white shadow-sm">
  ...
  return (
    <div className="inline-flex max-w-full items-center gap-2 rounded-full bg-neutral-900 px-3 py-2 text-sm font-black text-white shadow-sm">
...
function PurchaseLinkCard({
  ...
  return (
    <a
      ...
      className={`group flex min-h-[158px] flex-col justify-between rounded-3xl border p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg ${meta.shellClassName} ${isSingle ? "md:flex-row md:items-center md:gap-5" : ""}`}
    >
      <div className={isSingle ? "min-w-0 flex-1" : ""}>
      ...
      <span
        className={`mt-4 inline-flex w-fit shrink-0 items-center gap-2 rounded-full px-4 py-2 text-xs font-black transition group-hover:translate-x-0.5 ${meta.ctaClassName}`}
      >
...
export default function PurchaseLinksPanel({
  ...
  return (
    <section className="mt-6 rounded-3xl border border-amber-200 bg-gradient-to-br from-amber-50 via-white to-orange-50 p-4 shadow-sm md:p-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <h3 className="mt-1 text-lg font-black leading-tight text-neutral-950">
            Pintu belanja dari data internal Lighterracy
          </h3>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-neutral-600">
            Link pembelian dicek berdasarkan ISBN yang kamu scan atau ketik. Info buku boleh diperkaya dari sumber lain, tapi kanal belinya tetap mengikuti data internal.
          </p>
        </div>
      ...
      {!loading && !error && sortedLinks.length === 0 ? (
        <div className="mt-4 rounded-2xl border border-dashed border-amber-300 bg-white/80 px-4 py-4">
          ...
          <p className="mt-1 text-sm leading-6 text-neutral-600">
            Buku ini boleh jadi sudah dikenali, tapi Lighterracy belum punya data link pembelian untuk ISBN ini. Kamu tetap bisa copy ISBN atau cek toko terdekat dulu ya.
          </p>
        </div>
      ) : null}
