...
function parseIsOpen(hours?: string) {
  ...
  open.setHours(oh, om || 0, 0, 0);
  const close = new Date(now);
  close.setHours(ch, cm || 0, 0, 0);
...
export default function StoreDetailPage() {
  ...
  useEffect(() => {
    ...
    // jadwalkan setLoaded(false) agar tidak sync di body effect
    ...
    fetch("/data/stores.json")
      ...
      .then((list: Store[]) => {
        if (cancelled) return;
        setStore(list.find((s) => s.slug === slug) ?? null);
        setLoaded(true);
      })
      .catch(() => {
  ...
  return (
    <main className="mx-auto max-w-screen-md px-4 py-6 space-y-4">
      <Link href="/stores" className="text-sm text-brand">
        ← Kembali ke Stores
      </Link>
      ...
      <div className="rounded-2xl overflow-hidden border shadow-soft">
          ...
          {isOpen !== null && (
            <div
              className={`absolute top-2 right-2 text-xs px-2 py-1 rounded-full ${
                isOpen ? "bg-emerald-600" : "bg-gray-700"
              } text-white`}
            >
        ...
        <div className="p-4 space-y-2">
          ...
          <a href={maps} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-brand text-sm">
            Buka di Maps →
          </a>
        </div>
      </div>
