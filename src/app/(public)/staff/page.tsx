    ...
    async function load() {
      ...
      try {
        ...
        if (response.status === 401 || response.status === 403) {
          // Token invalid / session mati / role salah → bersihkan & lempar ke login
        ...
        if (!response.ok) {
          setState("error");
          setError(data?.message ?? "Gagal memuat data staff.");
          return;
        }
  ...
  return (
    <main className="min-h-dvh bg-[#f7f7f7] flex items-center justify-center px-4 py-10">
      <Card className="w-full max-w-md shadow-lg border border-zinc-200">
        <CardHeader>
          <CardTitle className="text-xl font-semibold">Staff Home</CardTitle>
          <CardDescription>
            Halaman ini hanya bisa diakses oleh staff yang sudah login dengan sesi aktif.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {isLoading && (
            <p className="text-sm text-zinc-700">Memuat data staff dari server...</p>
          )}

          {state === "success" && user && (
            <div className="space-y-2">
              <p className="text-sm text-emerald-700">
                {message ?? "Akses endpoint staff berhasil."}
              </p>
