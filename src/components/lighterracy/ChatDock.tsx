...
export default function ChatDock() {
  ...
  useEffect(() => {
    // greet once
    setMsgs([
      { role: "ai", time: nowText(), text: `${greeting()} Kak 👋 Aku Lightcy. ada yang bisa aku bantu kak, bahas buku apa nih? , atau kakak bisa gunakan tombol Scan ISBN di atas.` },
    ]);
  }, []);
  ...
  function mockReply(q: string) {
    ...
    if (lower.includes("anak")) return "Ini ide cepat: seri bergambar, board book, dan activity book. Coba ketik 'anak 3-5 tahun' ya.";
    if (lower.includes("nyt")) return "Cek bagian 'Rekomendasi New York Times' di atas—itu daftar best-seller minggu ini.";
    if (lower.includes("scan")) return "Klik tombol Scan ISBN di hero untuk menyalakan kamera. Aku akan bantu identifikasi kodenya.";
    return "Catat. Aku bantu merangkum & carikan opsi terkait begitu kita sambungkan ke data asli.";
  }
  ...
  return (
    <>
      <Button
        id="chat-trigger"
        onClick={() => setOpen((v) => !v)}
        className="fixed bottom-5 right-5 rounded-full h-12 w-12 bg-brand text-black shadow-soft"
        aria-label="Chat"
      >
        <MessageSquare />
      </Button>
      ...
      {open && (
        <div
          ref={panelRef}
          className="fixed bottom-20 right-5 w-[min(92vw,380px)] h-[520px] rounded-2xl border bg-background shadow-soft flex flex-col overflow-hidden"
        >
          {/* header */}
          ...
          <div className="flex-1 p-3 overflow-auto space-y-3">
           ...
           {typing && (
  <div
    className="max-w-[85%] rounded-2xl bg-muted p-3 text-sm flex items-center gap-2"
  ...
  >
    ...
    <span className="inline-flex items-center gap-1" role="status">
      ...
      <span className="w-2 h-2 rounded-full bg-gray-400 typing-dot" style={{ animationDelay: "400ms" }} />
          ...
          {/* input bar mock (klik chip saja untuk demo) */}
          <div className="h-12 border-t px-3 flex items-center gap-2">
            <input
              ref={inputRef}
              placeholder="Ketik (mockup)..."
              className="flex-1 h-8 text-sm rounded-md border px-2"
              onKeyDown={(e) => {
                if (e.key === "Enter") {
            ...
            />
            <button
              className="text-sm px-3 h-8 rounded-md bg-brand text-black"
              onClick={() => {
