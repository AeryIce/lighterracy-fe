...
export default function useUserLocation(options?: PositionOptions): GeoState {
  ...
  // set nilai awal SEKALIAN, jadi tidak setState sync di dalam effect
  ...
  useEffect(() => {
    ...
    const onSuccess = (p: GeolocationPosition) => {
      if (cancelled) return;
      setState({ loading: false, error: null, coords: { lat: p.coords.latitude, lng: p.coords.longitude } });
    };
    const onError = (err: GeolocationPositionError) => {
      if (cancelled) return;
      setState({ loading: false, error: err.message || "Location error", coords: null });
    };
